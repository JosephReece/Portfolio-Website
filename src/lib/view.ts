...
export const currentView = derived(page, ($page) => $page.params.view as View);
export function changeView(v: View) {
  goto(`/${v}`);
}

export const tooltip = writable<{ title: string; description: string } | null>(
  null,
);
...
export const groupings: Record<
  View,
  { label: string; position: Vector; target: Vector }
> = {
  home: {
    label: "Home",
    position: [3, 2.5, 3],
